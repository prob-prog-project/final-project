...
def run_gmm(stan_model,infer):
    ...
    GMM_model.exe_file
    GMM_model.code()
    GMM_fit = GMM_model.variational(data=GMM_data, output_dir='.',show_console=False,require_converged=False,tol_rel_obj=.001,iter=20000)
    return GMM_fit
...
def get_t_tests(ppd,data):
    ...
    return [scipy.stats.ttest_ind(deletion_d,deletion_p),scipy.stats.ttest_ind(nothing_p,nothing_d),scipy.stats.ttest_ind(amplification_p,amplification_d)]
...
def create_hpf_data(adata_subsampled,X,mixture_means):
    ...
    data['a_prime'] = 1
    data['b_prime'] = float(np.mean(np.sum(adata_subsampled.layers['counts'],axis=1))/np.std(np.sum(adata_subsampled.layers['counts'],axis=1)))
    data['a'] = .3

    data['c_prime'] = 1
    data['d_prime'] = float(np.mean(np.sum(adata_subsampled.layers['counts'],axis=0))/np.std(np.sum(adata_subsampled.layers['counts'],axis=0)))
    data['c'] = .3
...
def run_hpf(stan_model, adata_subsampled,X,mixture_means):
    ...
    mod_scHPF_fit = mod_scHPF_model.variational(data=mod_scHPF_data, output_dir='.',show_console=False,require_converged=False,tol_rel_obj=.001,iter=10000)
